...
class UserCreationForm extends React.Component<Props, State> {
  ...
  state = {
    isTouched: false,
    isSubmitted: false,
    values: { email: '', password: '', passwordRepeat: '', role: ROLE_USER, status: STATUS_ACTIVE },
    errors: { email: '', password: '', passwordRepeat: '', role: '', status: '' },
  };

  componentWillReceiveProps(nextProps: Props): void {
    const nextErrors = nextProps.user.backendUserCreationForm.errors;
  ...
  handleSubmitting = (event: SyntheticEvent<HTMLFormElement>): void => {
    ...
    this.props.userMethods.backendCreate(data);
  ...
  render(): React$Node {
    ...
    return (
      <form onSubmit={this.handleSubmitting} noValidate>
        ...
        <button className="btn btn-primary" disabled={this.state.isSubmitted}>Create</button>
      </form>
    );
  }
}
